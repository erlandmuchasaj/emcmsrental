	...
	public function initialize(Controller $controller) {
		...
		if ($maintenanceEnable === true) {
			...
			if (!$this->isAllowedAction($controller->request->params) &&
				!$this->isAdministrator($controller->request->params) &&
				// strstr(Router::url(null, true), $controller->request->webroot) &&
				(Router::url(null, true) !== $maintenanceUrl) &&
			...
			// on service
			if (Router::url(null, true) === $maintenanceUrl) {
				$controller->redirect('/');
			}
		}
	}
 ...
 * @return Boolean
 */
	public function isAdministrator($params) {
		if (!empty($params['prefix']) && ('admin' === $params['prefix'])) {
			if (AuthComponent::user()) {
				return (bool)('admin' === AuthComponent::user('role'));
			}
		}
		return false;
	}
