 ...
 * @author 		  Erland Muchasaj (Updated functions).
 * @link          www.copify.com
 ...
 */
	protected $paypalClassicApiVersion = '104.0'; //'204';
...
/**
 * API credentials - nvp payment type
 */
	protected $nvpPaymentType = 'Sale'; //Sale, Authorization or Order
...
/**
 * API credentials - Adaptive App ID - [APP-80W284485P519543T] <= ONLY SANDBOX
 */
...
/**
 * Live endpoint for Adaptive Payments API
 */
	protected $liveAdaptivePaymentsEndpoint = 'https://svcs.paypal.com/AdaptivePayments/';

/**
 * Sandbox endpoint for Adaptive Payments API
 */
	protected $sandboxAdaptivePaymentsEndpoint = 'https://svcs.sandbox.paypal.com/AdaptivePayments/';

/**
 * Live endpoint for Invoicing Service API
 */
	protected $liveInvoicingServiceEndpoint = 'https://svcs.paypal.com/Invoice/';

/**
 * Sandbox endpoint for Invoicing Service API
 */
	protected $sandboxInvoicingServiceEndpoint = 'https://svcs.sandbox.paypal.com/Invoice/';

/**
 * Live endpoint for Permissions Service API
 */
	protected $livePermissionsServiceEndpoint = 'https://svcs.paypal.com/Permissions/';

/**
 * Sandbox endpoint for Permissions Service API
 */
	protected $sandboxPermissionsServiceEndpoint = 'https://svcs.sandbox.paypal.com/Permissions/';
 ...
 * @var string
 */
	protected $verify_uri = 'https://ipnpb.paypal.com/cgi-bin/webscr';

/**
 * IPN verify URI - Sandbox
 * Sandbox Postback URL
 *
 * @var string
 */
	protected $sandbox_verify_uri = 'https://ipnpb.sandbox.paypal.com/cgi-bin/webscr';
 ...
 * @var array
 */
	protected $redirectErrors = [10411, 10412, 10422, 10445, 10486];
 ...
 **/
	public function __construct($config = []) {
		...
		$this->errorMessages = [
			// General Errors
			10002 => __('Authentication/Authorization Failed - Username/Password is incorrect'),
			10006 => __('Version error,	Version is not supported'),
			10008 => __('Security error, Security header is not valid'),
			10014 => __('The API call has been denied as it has exceeded the permissible call rate limit.'),
			10101 => __('This API is temporarily unavailable. Please try later.'),
			10404 => __('ReturnURL is missing. ReturnURL tag has no content.'),
			10405 => __('CancelURL is missing. CancelURL tag has no content.'),
			10409 => __('Express Checkout token was issued for a merchant account other than yours.'),
			10410 => __('Invalid token.'),
			...
			10413 => __('The totals of the cart item amounts do not match order amounts.'),
			10415 => __('A successful transaction has already been completed for this token.'),
			...
			10605 => __('Currency is not supported.'),
			...
			10800 => __('Your request is too long. Check URLs and other long strings.'),
 ...
 */
	public function getVerifiedStatus($email) {
		...
		if(in_array($parsed['responseEnvelope']['ack'], array('Success', 'SuccessWithWarning'))) {
			...
			throw new PaypalException(__d('paypal', 'Paypal Error' . $parsed['error'][0]['message']));
		} else {
 ...
 **/
	public function setExpressCheckout($order) {
		try {
			...
			// Handle the resposne
			if (isset($parsed['ACK']) && in_array($parsed['ACK'], ['Success', 'SuccessWithWarning'])) {
 ...
 **/
	public function doExpressCheckoutPayment($order, $token , $payerId) {
		try {
			...
			if (isset($order['ipn_url'])) {
				$keysToAdd['NOTIFYURL'] = $order['ipn_url'];
			}
 ...
 **/
	public function buildExpressCheckoutNvp($order) {
		...
		if (!isset($order['return']) || empty($order['return'])) {
			throw new PaypalException(__d('paypal' , 'Valid "return" url must be provided'));
		}

		if (!isset($order['cancel']) || empty($order['cancel'])) {
			throw new PaypalException(__d('paypal' , 'Valid "cancel" url must be provided'));
		}
		...
		$nvps = array(
			...
			'VERSION' => $this->paypalClassicApiVersion,
			'PAYMENTREQUEST_0_PAYMENTACTION' => $this->nvpPaymentType,
			'USER' => $this->nvpUsername,
			'PWD'  => $this->nvpPassword,
			'SIGNATURE' => $this->nvpSignature,
			...
			'PAYMENTREQUEST_0_DESC' => (EMCMS_strlen($order['description']) > 123) ? EMCMS_substr($order['description'], 0, 123).'...' : $order['description'],
			// 'SOLUTIONTYPE'  => 'Sole',
			// 'LOGOIMG' => Router::url('/img/emcmsrental.png', true),
			// 'NOTETOBUYER' => 'Powered by EMCMS Rental Script.',
		);

		// Custom field?
		if (isset($order['custom']) && !empty($order['custom'])) {
			$nvps['PAYMENTREQUEST_0_CUSTOM'] = substr(strip_tags($order['custom']), 0, 250).'...';
		}

		// Locale of the pages displayed by PayPal during Express Checkout
		if (isset($order['locale']) && !empty($order['locale'])) {
			$nvps['LOCALECODE'] = $order['locale'];
		}

		// A label that overrides the business name in the PayPal account on the PayPal hosted checkout pages.
		if (Configure::check('Website.name')) {
			$nvps['BRANDNAME'] = Configure::read('Website.name');
		}
		...
		if (isset($order['items']) && is_array($order['items'])) {
			...
			foreach ($order['items'] as $m => $item) {
				...
				$nvps["L_PAYMENTREQUEST_0_NAME$m"] = substr(strip_tags($item['name']), 0, 120);

				// Description an Tax are optional
				if (array_key_exists("description", $item)) {
					$nvps["L_PAYMENTREQUEST_0_DESC$m"] = substr(strip_tags($item['description']), 0, 120);
				}

				// Item subtotal
				$nvps["L_PAYMENTREQUEST_0_AMT$m"] = number_format($item['subtotal'], 2, '.', '');

				// Item Number - optional  Each item can be further identified by an item number.
				// If the item is an eBay auction item, it is recommended that you provide the eBay item number in this field.
				if (array_key_exists("number", $item)) {
					$nvps["L_PAYMENTREQUEST_0_NUMBER$m"] = (int) $item['number'];
				}
				...
				if (array_key_exists("tax", $item)) {
					$nvps["L_PAYMENTREQUEST_0_TAXAMT$m"] = number_format($item['tax'], 2, '.', '');
 ...
 */
	public function formatRefundTransactionNvps($refund) {
		...
		if (!isset($refund['transactionId']) || empty($refund['transactionId'])) {
		...
		// Type of refund Default: Full
		$REFUNDTYPE = 'Full';
		if (isset($refund['type']) && in_array($refund['type'], ['Full', 'Partial'])) {
		...
		$nvps = [
			...
			'REFUNDTYPE' 	=> $REFUNDTYPE,				// Full - (Default), Partial
		];

		// Refund amount, only set if REFUNDTYPE is Partial
		if ($REFUNDTYPE === 'Partial') {
			// Amount to refund
			if (!isset($refund['amount']) || empty($refund['amount'])) {
		...
		// PayPal PAYER ID
		if (isset($refund['payer_id']) && !empty($refund['payer_id'])) {
			$refund['payer_id'] = preg_replace("/\s/" , "" , $refund['payer_id']);
			$nvps['PAYERID'] = $refund['payer_id'];
		}

		// Set note
		if (isset($refund['note']) && !empty($refund['note'])) {
			$nvps['NOTE'] = substr(trim($refund['note']), 0, 255); // Up to 255 characters of information displayed to customer
		}
 ...
 *
 * @return array Formatted array of Paypal NVPs for Accont Balance
	...
	public function GetBalance() {
		try {
			...
			if (isset($parsed['ACK']) && in_array($parsed['ACK'], array('Success', 'SuccessWithWarning')))  {
				return $parsed;
			} else if (isset($parsed['ACK']) && $parsed['ACK'] == "Failure" && isset($parsed['L_LONGMESSAGE0']))  {
				throw new PaypalException($this->getErrorMessage($parsed));
			} else {
				throw new PaypalException(__d('paypal' , 'There was an error while connecting to Paypal'));
			}
 ...
 * and other informaton about your account. You need the encrypted account number when working with
 *
 * @return array Formatted array of Paypal NVPs for PayPal Info
 * @author Erland Muchasaj
 */
 ...
 *
 * @return array Formatted array of Paypal NVPs for a Transaction
 * @author Erland Muchasaj
 */
...
/**
 * doMassPayment();
 * The doMassPayment API operation To send multiple payment to different recieps
 * A single MassPayRequest can include up to 250 MassPayItems.
 * Format of the array for mass payment
 * e.g.
 ...
 *	$MassPay = array(
 *		'currency'		=> 'EUR',
 *		'email_subject' => 'Massive Payment', // optional
 *	);
 *	$single_item = array(
 *		'email' => 'erlandi_20-buyer@hotmail.com',
 *		'amount'=> 150,
 *		'note' 	=> 'Hello Mate, this is a note for you', // optionl
 *	);
 *	$single_item1 = array(
 *		'email' => 'ermir.papadhimitri-buyer@atis-shpk.com',
 *		'amount'=> 80,
 *		'note' 	=> 'Lorem ipsum dolor',
 *	);
 *	$MassPay['item_type'][] = $single_item;
 *	$MassPay['item_type'][] = $single_item1;
 *	try {
 *		$returnedArray = $this->Paypal->doMassPayment($MassPay);
 *	} catch (Exception $e) {
 ...
 * @return array Details of the completed transaction
 * @author Erland Muchasaj
 * @link(Erland Muchasaj, http://erlandmuchasaj.com)
 */
	public function doMassPayment($order) {
		try {
			// Build the NVPs
			$nvps = $this->buildMassPaymentNvp($order);
 ...
 *
 * @param array $order Takes an array order (See tests for supported fields).
 * @return array Formatted array of Paypal NVPs for MassPaymen
 * @author Erland Muchasaj
 **/
	public function buildMassPaymentNvp($order) {
		if (!isset($order) || empty($order) || !is_array($order)) {
			throw new PaypalException(__d('paypal' , 'You must pass a valid order array'));
		}
		...
		$nvps = array(
			...
			'PWD' 		=> $this->nvpPassword,
			'SIGNATURE' => $this->nvpSignature,
			'RECEIVERTYPE'=>'EmaiAlddress',		 // Must be EmailAddress, PhoneNumber, or UserID;
			'CURRENCYCODE'=> $order['currency'], // A single request must include items that are of the same currency
		);
		// Custom field?
		if (isset($order['email_subject']) && !empty($order['email_subject'])) {
			$nvps['EMAILSUBJECT'] = $order['email_subject'];
		}
		// Add up each item and calculate totals
		if (isset($order['item_type']) && !empty($order['item_type']) && is_array($order['item_type'])) {
			// Build each item
			foreach ($order['item_type'] as $m => $item) {
				//  Email address of recipient.
				$nvps["L_EMAIL$m"] = $item['email'];

				// (Required) Payment amount
				$nvps["L_AMT$m"] = $item['amount'];

				// (Optional) Custom note for each recipient.
				if (array_key_exists("note", $item)) {
					$nvps["L_NOTE$m"] = $item['note'];
				}
			}
		}
		return $nvps;
...
/**
 * Search information about a transactions.
 *
 * @param array original transaction details key=>value pared
 * @return array Formatted array of Paypal NVPs for Transactions
 * @author Erland Muchasaj
 */
	public function TransactionSearch($search) {
		try {
			$nvps = $this->buildTransactionSearchNvp($search);
...
/**
 * Formats the search array to Paypal nvps
 *
 * @param array $search Takes an array search (See tests for supported fields).
 * @return array Formatted array of Paypal NVPs for TransactionSearch
 * @author Erland Muchasaj
 **/
	public function buildTransactionSearchNvp($search) {
		if (!isset($search) || empty($search) || !is_array($search)) {
			throw new PaypalException(__d('paypal' , 'You must pass a valid search array'));
		}

		// Transcation start date
		if (!isset($search['start_date']) || empty($search['start_date'])) {
			throw new PaypalException(__d('paypal' , 'Start date is Required'));
		}
		...
		if (isset($search['end_date']) && !empty($search['end_date'])) {
			$nvps['ENDDATE'] = $search['end_date'];
		}

		// Search by the buyer's email address.
		if (isset($search['buyer_email']) && !empty($search['buyer_email'])) {
			$nvps['EMAIL'] = $search['buyer_email'];
		}

		// Search by transaction amount.
		if (isset($search['buyer_firstname']) && !empty($search['buyer_firstname'])) {
			$nvps['FIRSTNAME'] = $search['buyer_firstname'];
		}

		// Search by transaction amount.
		if (isset($search['buyer_lastname']) && !empty($search['buyer_lastname'])) {
			$nvps['LASTNAME'] = $search['buyer_lastname'];
		}

		//  Search by the receiver's email address.
		if (isset($search['reciver_date']) && !empty($search['reciver_date'])) {
			$nvps['RECEIVER'] = $search['reciver_date'];
		}

		// Transcation id
		if (isset($search['transaction_id']) && !empty($search['transaction_id'])) {
			$nvps['TRANSACTIONID'] = $search['transaction_id'];
		}

		// Search by transaction amount.
		if (isset($search['amount']) && !empty($search['amount'])) {
			$nvps['AMT'] = $search['amount'];
		}

		// Search by transaction amount.
		if (isset($search['currency']) && !empty($search['currency'])) {
			$nvps['CURRENCYCODE'] = $search['currency'];
		}
		...
		if (isset($search['transaction_class']) && !empty($search['transaction_class'])) {
			$nvps['TRANSACTIONCLASS'] = $search['transaction_class'];
		}
...
/**
 * Returns Paypal Adaptive Payments API endpoint
 *
 * @author Erland Muchasaj
 * @return string
 **/
	public function getAdaptivePaymentsEndpoint() {
		if ($this->sandboxMode) {
			return $this->sandboxAdaptivePaymentsEndpoint;
		}
		return $this->liveAdaptivePaymentsEndpoint;
	}

/**
 * Returns Paypal Invoicing Service API endpoint
 *
 * @author Erland Muchasaj
 * @return string
 **/
	public function getInvoicingServiceEndpoint() {
		if ($this->sandboxMode) {
			return $this->sandboxInvoicingServiceEndpoint;
		}
		return $this->liveInvoicingServiceEndpoint;
	}

/**
 * Returns Paypal Permissions Service API endpoint
 *
 * @author Erland Muchasaj
 * @return string
 **/
	public function getPermissionsServiceEndpoint() {
		if ($this->sandboxMode) {
			return $this->sandboxPermissionsServiceEndpoint;
		}
		return $this->livePermissionsServiceEndpoint;
	}
 ...
 */
	public function getPaypalUri() {
	    if ($this->sandboxMode) {
	        return $this->sandbox_verify_uri;
	    }
 ...
 */
	public function verifyIPN() {
	    if (!count($_POST)) {
	        throw new PaypalException(__d('paypal' , 'Missing POST Data'));
	    }
	    ...
	    if ($this->use_local_certs) {
	        curl_setopt($ch, CURLOPT_CAINFO, __DIR__ . DS . 'cert'.DS.'cacert.pem');
	    }
