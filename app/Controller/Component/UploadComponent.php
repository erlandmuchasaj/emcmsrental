 ...
 */
	public function validateUpload($file = array(), $directories = array()) {
		$response = array(
			'is_valid' => true,
			'message'  => ''
		);

		if (!isset($file['file']['name']) || empty($file['file']['name'])) {
			$response['is_valid'] = false;
			$response['message'] = __('You must upload a file before you continue!');
			return $response;
		}

		if (!$this->isRealImage($file['file']['tmp_name'], $file['file']['type']) || !$this->isCorrectImageFileExt($file['file']['name'], $this->ALLOWED_EXTENTIONS) || preg_match('/\%00/', $file['file']['name'])) {
			$response['is_valid'] = false;
			$response['message'] = __('Image format not recognized, allowed formats are: .gif, .jpg, .png.');
			return $response;
		}

		$errorMessage = $this->checkUploadError($file['file']["error"]);
		if ($errorMessage) {
			$response['is_valid'] = false;
		...
		if (preg_match('/\%00/', $file['file']['name'])) {
			$response['is_valid'] = false;
		...
		$extension = $this->strtolower(pathinfo($file['file']['name'], PATHINFO_EXTENSION));
		...
		if ($file['file']['size'] > intval($this->FILE_LIMIT)) {
			$response['is_valid'] = false;
			$response['message'] = __('File (size : %s) is biger then the limit! (max : %s)', $file['file']['size'],  $this->__humanFileSize($this->FILE_LIMIT));
			return $response;
		}

		if (file_exists($directories['or'].$file['file']['name']) && is_file($directories['or'].$file['file']['name'])) {
			$response['is_valid'] = false;
 ...
 * @param array $file,
 * @param array $directories,
 * @param boolean $use_thumbnails,
 * @return array if everything went OK
 */
	public function uploadFile($file, $directories = [], $use_thumbnails = true) {
		$response = array(
			'is_valid' => false,
		...
		if (isset($file['file']) && !empty($file['file']['tmp_name'])) {
			...
			$mainFile	= $directories['or'].$full_name;
			...
			if (is_uploaded_file($file['tmp_name'])) {
				if (($tmpName = tempnam($this->TMP_IMG_DIR, 'EMCMS')) && move_uploaded_file($file['tmp_name'], $mainFile)) {
					...
					if ($use_thumbnails === true) {
						$this->__createThumbnailUpload($full_name, $this->XSMALL_THUMBNAIL,$directories['or'], $directories['xs']);
						$this->__createThumbnailUpload($full_name, $this->SMALL_THUMBNAIL, $directories['or'], $directories['sm']);
						$this->__createThumbnailUpload($full_name, $this->MEDIUM_THUMBNAIL,$directories['or'], $directories['md']);
						$this->__createThumbnailUpload($full_name, $this->LARGE_THUMBNAIL, $directories['or'], $directories['lg']);
						$this->__createThumbnailUpload($full_name, $this->XLARGE_THUMBNAIL,$directories['or'], $directories['xl']);
					}
					$response = array(
						'is_valid' => true,
						'message'  => '',
