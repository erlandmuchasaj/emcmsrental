	...
	<div class="col-sm-12">
		<div class="panel panel-em">
			<header class="panel-heading">
				<?php echo $this->Html->tag('h4',__('Waiting admin approvement'),array('class' => 'display-inline-block')) ?>
			</header>
								...
								<tr id="arrayorder_<?php echo $elementId; ?>" class="parent-element">
									<td data-th="<?php echo __('Thumbnail');?>" class="sort-handle image">
										<?php
											...
											echo $this->Html->tag('div', $img . $this->Html->tag('span', __('%s', $type), array('class' => 'badge label-success payed-badge'))
									...
									<td class="actions">
										...
										<?php
											...
											echo $this->Html->link($this->Html->tag('i','', array('class' => 'fa fa-check')).'&nbsp;'.__('Approve'), 'javascript:void(0);',array('escape'=>false, 'onclick' =>"ShowConfirmApprove({$elementId},'{$var}', this); return false;",'id'=>'element_'.$elementId, 'class'=>'label label-success'));
										?>
									</td>
								</tr>
							<?php endforeach; ?>
...
<script>
/**
 * [ShowConfirmApprove Opem Confirmation box for when delating a row, Ajax]
 ...
 */
	function ShowConfirmApprove(id, name) {
		...
		$('#lblMsgConfirmYesNo').html('<?php echo __('Are you sure you want to approve'); ?>: <b>' + name + '</b>?');
		$('#btnYesConfirmYesNo').off('click').on('click', function() {
			$.ajax({
				url:'<?php echo Router::url(array('admin' => true, 'controller' => 'properties', 'action' => 'approve')); ?>/'+ id,
				type: 'POST',
				dataType: 'json',
