<?php echo $this->Form->create('AccommodationType'); ?>
<article class="panel panel-em">
	<header class="panel-heading">
		<h4><?php echo __('Add Accommodation Type'); ?></h4>
	</header>
				...
				<?php foreach ($languages as $index => $language): ?>
					...
					<?php endif ?>
						<?php
							...
							echo $this->Form->input('AccommodationType.' . $id_lang . '.language_id', ['type' => 'hidden', 'value' => $id_lang]);
							
							echo $this->Form->input('AccommodationType.' . $id_lang . '.accommodation_type_id', ['type' => 'hidden', 'value' => $accommodation_type_id]);

							echo $this->Form->input('AccommodationType.' . $id_lang . '.accommodation_type_name', $options); 
						?>
					<?php if (count($languages) > 1): ?>
						</div>
