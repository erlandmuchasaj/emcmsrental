...
<article class="panel panel-em">
    <header class="panel-heading">
    	<?php echo $this->Html->tag('h4', __('Add Safety'), array('class'=>''))?>
    </header>
    <div class="panel-body">
    	<?php echo $this->Form->create('Safety', array('type'=>'file','class'=>'form-horizontal')); ?>
			...
			<div class="col-sm-6">
				...
				<?php foreach ($languages as $index => $language): ?>
					...
					<?php 
						...
						echo $this->Form->input('SafetyTranslation.' . $id_lang . '.language_id', ['type' => 'hidden', 'value' => $id_lang]); 

						echo $this->Form->input('SafetyTranslation.' . $id_lang . '.safety_name', $options); 
					?>
					<?php if (count($languages) > 1): ?>
						</div>
						<div class="col-lg-2 dropdown">
							<button type="button" class="btn btn-default dropdown-toggle" tabindex="-1" data-toggle="dropdown">
								<?php echo h($language['Language']['language_code']); ?>&nbsp;<i class="caret"></i>
							</button>
							<ul class="dropdown-menu">
								<?php foreach ($languages as $key => $language): ?>
								<li>
									<a href="javascript:hideOtherLanguage(<?php echo $language['Language']['id']; ?>);" tabindex="-1">
