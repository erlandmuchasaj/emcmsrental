...
<article class="panel panel-em">
    <header class="panel-heading">
        <?php echo $this->Html->tag('h4', __('Edit Safety'), array('class'=>''))?>
    </header>
    <div class="panel-body">
    	<?php 
			echo $this->Form->create('Safety',array('type'=>'file','class'=>'form-horizontal')); 
			echo $this->Form->input('Safety.id'); 
    	?>
            ...
            <div class="col-md-9">
				<div class="fileinput fileinput-new" data-provides="fileinput">
					<div class="fileinput-new thumbnail" style="max-width: 200px; max-height: 150px;">
						<?php
							if (file_exists($directory.$this->Form->value('Safety.icon')) && is_file($directory.$this->Form->value('Safety.icon'))) { 	
								 echo $this->Html->image('uploads/Safety/'.$this->Form->value('Safety.icon'), array('alt' => 'icon')); 
							} else {	
					...
					<div>
						<span class="btn btn-default btn-file">
							...
							<?php echo $this->Form->input('Safety.icon', array('type'=>'file','class'=>'default','label'=>false, 'div'=>false,));?>
						</span>
        ...
        <div class="form-group">
            ...
            <div class="col-sm-6">
                <div class="icon-picker" data-pickerid="fa" data-iconsets='{"fa":"<?php echo __("Select Icon"); ?>"}'>
					<?php echo $this->Form->input('Safety.icon_class', array('type'=>'hidden')); ?>
				</div>                      
            </div>
        </div>
        ...
        <div class="form-group">
        	<label class="control-label col-sm-3 required">
        		<span class="label-tooltip" data-toggle="tooltip" data-html="true" title="" data-original-title="<?php echo __('Invalid characters <>;=#{}');?>"> <?php echo __('Safety name');?>
        		</span>
        	</label>
        	<div class="col-sm-6">
        		...
        		<?php foreach ($this->request->data['SafetyTranslation'] as $index => $safety): ?>
        			<?php $id_lang = $safety['language_id']; ?>

        			<div class="translatable-field lang-<?php echo $id_lang; ?>" <?php echo ($safety['Language']['is_default'] != 1) ? 'style="display: none;"': ''; ?> >
        			<?php if (count($languages) > 1): ?>
        				<div class="col-lg-9">
        			<?php endif ?>
    					<?php 
    						$options = [
    							...
    							'label' => false,
    							'div' => false,
    							// 'value' => h($safety['safety_name']),
    							'required' => 'required',
    							'placeholder' => __('Safety name'), 
    							'data-minlength' => '0', 
    							'data-maxlength' => '140',
    						];

							// if ($safety['Language']['is_default'] == 1) {
							// 	$options['required'] = 'required';
							// }

    						echo $this->Form->input('SafetyTranslation.' . $index . '.id', ['type' => 'hidden', /*'value' => $safety['id']*/]);

    						echo $this->Form->input('SafetyTranslation.' . $index . '.language_id', ['type' => 'hidden', /*'value' => $safety['language_id']*/]); 

    						echo $this->Form->input('SafetyTranslation.' . $index . '.safety_id', ['type' => 'hidden', /*'value' => $safety['safety_id']*/]); 

    						echo $this->Form->input('SafetyTranslation.' . $index . '.safety_name', $options); 
    					?>
        			<?php if (count($languages) > 1): ?>
        				</div>
        				<div class="col-lg-2 dropdown">
        					<button type="button" class="btn btn-default dropdown-toggle" tabindex="-1" data-toggle="dropdown">
        						<?php echo h($safety['Language']['language_code']); ?>&nbsp;<span class="caret"></span>
        					</button>
