<?php
	$directory = Configure::read('App.www_root'). 'img' .DS. 'uploads' .DS. 'Safety'. DS;
	// echo $this->Html->css('icon-picker', null, array('block'=>'cssMiddle'));
?>
<div class="box box-danger" id="elementToMask">
	<div class="box-header with-border">
		<?php echo $this->Html->tag('h4', __('Safeties'), array('class'=>'box-title'))?>
		<div class="box-tools pull-right">
			<?php
				echo $this->Html->link($this->Html->tag('i','',array('class' => 'fa fa-plus-square')).'&nbsp;'.__('New Safety'),array('admin'=>true, 'controller' => 'safeties', 'action' => 'add'),array('escape' => false, 'class'=>'btn btn-default btn-sm')); 
			?>
		</div>
	</div>
			...
			<table class="table">
				<thead>
					<tr>
						...
						<th><?php echo $this->Paginator->sort('SafetyTranslation.safety_name', __('Name')); ?></th>
				...
				<tbody>
					<?php foreach ($safeties as $safety): ?>
						<tr class="parent-element">
							<td  data-th="<?php echo __('ID');?>"><?php echo h($safety['Safety']['id']); ?></td>
							<td  data-th="<?php echo __('Thumbnail');?>" class="avatar-img">
							<?php 
								if (file_exists($directory.$safety['Safety']['icon']) && is_file($directory .$safety['Safety']['icon'])) {
									echo $this->Html->image('uploads/Safety/'. h($safety['Safety']['icon']), array('alt' => 'icon', 'class'=>'img-responsive', 'width'=>35, 'height'=>35)); 
								} else {
									echo $this->Html->image('no-img-available.png', array('alt' => 'img', 'width'=>35, 'height'=>35));
								}
							?>
							</td>
							<td  data-th="<?php echo __('Icon');?>" class="icon-preview"><i class="<?php echo h($safety['Safety']['icon_class']); ?>"></i></td>
							<td  data-th="<?php echo __('Name');?>"><?php echo h($safety['SafetyTranslation']['safety_name']); ?></td>
							<td class="actions">
							<?php 
								echo $this->Html->link($this->Html->tag('i', '', array('class' => 'fa fa-eye')).'&nbsp;'.__('View'),array('admin'=>true,'controller'=>'safeties','action' => 'view', $safety['Safety']['id']),array('escape' => false, 'class'=>'btn btn-success btn-sm'));

								echo "&nbsp;";
								echo $this->Html->link($this->Html->tag('i', '', array('class' => 'fa fa-edit')).'&nbsp;'.__('Edit'),array('admin'=>true,'controller'=>'safeties','action' => 'edit', $safety['Safety']['id']),array('escape' => false, 'class'=>'btn btn-info btn-sm'));

								echo "&nbsp;";
								$var = h(addslashes($safety['SafetyTranslation']['safety_name']));
								$elementId = (int)h($safety['Safety']['id']);
	...
	function ShowConfirmDelete(id, name, obj) {
		...
		$('#btnYesConfirmYesNo').off('click').on('click',function () {
			...
			$.ajax({
				...
				url:'<?php echo Router::url(array('admin' => true,'controller' => 'safeties','action' => 'delete')); ?>/'+ id,
				type: 'POST',
				async: false,
				dataType: 'json',
				data:{data:{Safety:{id:id,safety_name:name}}},
				beforeSend: function() {
